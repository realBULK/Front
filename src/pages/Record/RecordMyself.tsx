  ...
  return (
    <div className="flex flex-col h-screen pt-[28px] px-[20px]">
      ...
      {customFoodBoxes.map((diet) => (
        <CustomFoodBox key={diet.id} {...diet} openMdoal={() => openModal(diet.id)} />
      ))}

      {isOpen && (
        <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 bg-[#F5F5F5] w-[393px] h-[429px] rounded-t-[30px] px-[20px]">
          <div className="flex justify-between items-baseline mt-[31px] ms-[15px] me-[10px] mb-[27px]">
            ...
            <div className="flex felx-row items-center">
              ...
              <p className="ms-1.5 text-[14px] font-[500]">후기보러가기</p>
            </div>
          </div>
          ...
          <div className="flex flex-row justify-between">
            ...
            <div className="flex flex-col w-[29%] justify-between ">
              <button
                className={`flex flex-row justify-center items-center py-4 rounded-[15px] shadow-whiteBox h-[37px] border-[1px] border-solid border-[#EDEDED] opacity-80 ${isActive ? 'bg-[#ffffff]' : 'bg-red-100'}`}
                onClick={handleActive}
              >
                <p className="text-[16px] font-[500]">인분({modalInfo.unit}g)</p>
              </button>
              <button
                className={`flex flex-row justify-center items-center px-[22px] py-4  rounded-[15px] shadow-whiteBox h-[37px] border-[1px] border-solid border-[#EDEDED] opacity-80 ${!isActive ? 'bg-[#ffffff]' : 'bg-[rgba(242, 225, 218, 0.80]'}`}
                onClick={handleActive}
              >
